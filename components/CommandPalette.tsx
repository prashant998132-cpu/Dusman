    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          onClick={onClose}
          style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '15vh' }}
        >
          <motion.div
          ...
          >
            <input
              ...
              style={{ width: '100%', padding: '16px 20px', background: 'none', border: 'none', borderBottom: '1px solid rgba(255,26,136,0.15)', color: 'var(--text)', fontSize: 15, outline: 'none', boxSizing: 'border-box' }}
            />
            <div style={{ maxHeight: 280, overflowY: 'auto' }}>
              {filtered.length === 0
                ? <div style={{ padding: '16px 20px', color: 'var(--muted)', fontSize: 13 }}>No commands found</div>
                : filtered.map(cmd => (
                  <div
                    key={cmd.key}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--muted)' }}>{cmd.key}</div>
                    </div>
                  </div>
                ))
              }
            </div>
            <div style={{ padding: '8px 16px', borderTop: '1px solid rgba(255,255,255,0.05)', fontSize: 11, color: 'var(--muted)', display: 'flex', gap: 12 }}>
